...
from csh_clip import init_network, create_data_batch, move_to_cuda, detection_filter, vis_gt
...
class MGNService:
    def __init__(self):
        ...
        self.cam = RealSenseCamera(device_id=143322071682)
        self.pipe = self.cam.connect()
        logging.info('Done')
    ...
    def init_network(self):
        ...
        Network = MGN(32, class_agnostic=True, feat_name=self.args.net,         
                        feat_list=('conv' + conv_num,), pretrained=True)
        Network.create_architecture()

        # Loading weight                                                              
        if self.args.resume:                                                                   # 학습할 때, 이전에 저장한 체크포인트 파일을 불러와서 모델의 가중치를 복원하는 역할
            output_dir = 'output/vmrdcompv1/res101'         
            load_name = os.path.join(output_dir,
                                    self.args.frame + '_{}_{}_{}.pth'.format(self.args.checksession, self.args.checkepoch,
    ...
    def create_data_batch(self, rgb_image):
    ...
    def detection_filter(self, all_boxes, all_grasp=None, max_per_image=10):
    ...
    def vis_gt(self, data_list):
        ...
        o_inds, object_data, g_inds, grasp_data = self.draw_graspdet_with_owner(data_list[2].cpu().numpy(),
                                        data_list[3].cpu().numpy(), data_list[6].cpu().numpy())
        return im_vis, o_inds, object_data, g_inds, grasp_data

    def draw_graspdet_with_owner(self, o_dets, g_dets, g_inds):
        """
    ...
    def compute_service_handler(self, req):
        try:
            ...
            while i <= 10:
                ...
                data_batch = self.move_to_cuda(data_batch)

                # Forward process
                with torch.no_grad():
                    rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_box, loss_cls, loss_bbox, rois_label, grasp_loc, grasp_prob, \
                grasp_bbox_loss, grasp_cls_loss, grasp_conf_label, grasp_all_anchors = self.Network(data_batch)
                boxes = rois[:, :, 1:5]
                ...
                det_box, det_grasps = self.detection_filter(det_box, det_grasps, max_per_image)
                ...
                im_vis, o_inds, object_data, g_inds, grasp_data = self.vis_gt(data_list)
        ...
        pixel_x, pixel_y = map(int, [(x1 + x3)/2, (y1 + y3)/2])
        pos_z = raw_depth[pixel_y, pixel_x] * self.cam_depth_scale - 0.015
        pos_x = np.multiply(pixel_x - self.cam.intrinsics.ppx,
                            pos_z / self.cam.intrinsics.fx)
        pos_y = np.multiply(pixel_y - self.cam.intrinsics.ppy,
                            pos_z / self.cam.intrinsics.fy)
