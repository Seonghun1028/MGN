...
class dataViewer(object):
    def __init__(self, classes):
        ...
        self.color_pool = (math.ceil(self.num_classes / len(self.color_pool)) + 1) * self.color_pool
    ...
    def draw_single_grasp(self, img, grasp, test_str=None, text_bg_color=(255, 0, 0)):
        ...
        if test_str is not None:
            ...
            text_w = 17 * text_len
            gtextpos = (int(gr_c[0] - text_w / 2), gr_c[1] + 20)         ### int 추가
            gtext_lu = (int(gr_c[0] - text_w / 2), gr_c[1])              ### int 추가
            gtext_rd = (int(gr_c[0] + text_w / 2), gr_c[1] + 25)         ### int 추가
    ...
    def draw_graspdet(self, im, dets, g_inds=None):
        ...
        dets = dets[(dets[:,:8].sum(-1)) > 0].astype(int)
    ...
    def draw_objdet(self, im, dets, o_inds = None):
        ...
        dets = dets[(dets[:,:4].sum(-1)) > 0].astype(int)
