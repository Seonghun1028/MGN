...
// #include <THC/THC.h>                                                                  // 주석 처리
#include <THC/THCAtomics.cuh>
#include <THC/THCDeviceUtils.cuh>

__host__ __device__ inline int64_t ATenCeilDiv(int64_t a, int64_t b) {                   // 추가
  return (a + b - 1) / b;
}
...
at::Tensor ROIAlign_forward_cuda(const at::Tensor& input,
  ...
  dim3 grid(std::min(ATenCeilDiv(output_size, 512L), 4096L));                                          // 수정
  dim3 block(512);

  if (output.numel() == 0) {
    //THCudaCheck(cudaGetLastError());
    cudaError_t err = cudaGetLastError();                                                              // 수정 
    if (err != cudaSuccess) {
        AT_ERROR("Error: ", cudaGetErrorString(err));
    }
    return output;
  }
...
at::Tensor ROIAlign_backward_cuda(const at::Tensor& grad,
  ...
  dim3 grid(std::min(ATenCeilDiv(grad.numel(), 512L), 4096L));                                       // 수정
  dim3 block(512);
