...
from __future__ import absolute_import  # 상대경로 대신 항상 절대경로를 사용하도록 설정
from __future__ import division         # 나누기 연산자(/)가 항상 실수 나누기를 수행
from __future__ import print_function   # print문을 함수 호출의 구문으로 사용하도록 설정
...
#################################################################################################
        ...
        conv_num = str(int(np.log2(cfg.RCNN_COMMON.FEAT_STRIDE[0])))                              # conv_num = 4 
...
#########################################################################################################################
            ...
            if args.vis:
                vis_boxes, vis_grasps = objgrasp_inference(cls_prob[0].data,
                                                           ...
                                                           boxes[0].data,
                                                           class_agnostic=args.class_agnostic,
                                                           g_box_prior=grasp_all_anchors.data, for_vis=True, topN_g=1)
...
def test():
    ...
    # init output directory for model saving
    output_dir = args.save_dir + "/" + args.dataset + "/" + args.net            ### output/vmrdcompv1/res101
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    _, _, _, _, cls_list = combined_roidb(args.imdbval_name, training=False)    ### args.imdbval_name = vmrd_compv1_test
